...
class MultiModeSeq2SeqModel1(nn.Module):
    def __init__(self, input_size, num_classes, window_size, ViT_feature_dim=256, hidden_dim=32,dropout=0.1,vit_type="vit_tiny_patch16_224",weight_path=None):
        ...
        self.decoder = nn.Sequential(
            nn.Linear(hidden_dim + ViT_feature_dim, 32),
            nn.ReLU(),
            nn.Linear(32, 16),
            nn.ReLU(),
    ...
    def forward(self, x, image_features):
        ...
        # 模态融合
        fused_features = torch.cat([encoded.transpose(1, 2), image_expand_features.transpose(1, 2)], dim=-1)  # (batch_size, window_size, hidden_dim)
...
def main(
    ...
    model = MultiModeSeq2SeqModel1(input_size, num_classes, window_size,
                                   ViT_feature_dim=hidden_dim, hidden_dim=hidden_dim,
