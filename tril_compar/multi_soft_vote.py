...
class ViTSeq2Seq(nn.Module):
    def __init__(self, num_classes, window_size, ViT_feature_dim=256,vit_type="vit_tiny_patch16_224",weight_path=None):
...
def train_model(model, train_loader, valid_loader, num_classes,device, patience=8, factor=0.5,
                ...
                save_model_name="./best_model.pth",mode="img"):
    if alpha is None:
...
def evaluate_seq2seq_model(img_model,numeric_model, device, data_loader):
    img_model.eval()
    numeric_model.eval()
    all_preds = []
    all_labels = []
    ...
    with torch.no_grad():
        for batch_X, batch_img, batch_y, batch_idx in data_loader:
            ...
            batch_images = batch_img.float().to(device)
            # 提取图像特征
            image_features = img_model.image_feature_extractor(batch_images).to(device)
            outputs1 = img_model(image_features)
            outputs2 = numeric_model(batch_X)
...
def plot_loss_curves(train_losses1, test_losses1, train_losses2, test_losses2,save_path):
    """
    绘制当前随机数种子的训练和验证损失
    train_loss:训练损失
    test_loss:测试损失
    ...
    plt.plot(train_losses1, label='Training loss img', linewidth=2)
    plt.plot(test_losses1, label='Test loss img', linewidth=2)
    plt.plot(train_losses2, label='Training loss numeric', linewidth=2)
    plt.plot(test_losses2, label='Test loss numeric', linewidth=2)
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
...
def main(
        ...
        lr=0.0008,
        max_workers=50,
        hidden_dim_numeric=64,
        vit_type="vit_small_patch16_224",
    ...
    img_model = ViTSeq2Seq(num_classes, window_size,
                           ViT_feature_dim=hidden_dim_img,
                           vit_type=vit_type,
                           weight_path="../img_encoder_weight/{vit_type}/model.safetensors")

    numeric_model = SimpleCNNSeq2Seq(input_size=input_size,
                                        num_classes=num_classes,
                                        window_size=window_size,
                                        hidden_dim=hidden_dim_numeric,
                                        dropout=dropout)

    img_model = img_model.to(device)
    numeric_model = numeric_model.to(device)
    print("====图像模态模型训练====")
    train_losses1, valid_losses1, _ = train_model(img_model, train_loader, valid_loader, num_classes, device, patience=8, factor=0.5,
                max_norm=0.5, alpha=None, gamma=2, weight_decay=1e-4, num_epochs=num_epochs, learning_rate=lr,
                save_model_name="",mode="img")
    print("====数值模态模型训练====")
    train_losses2, valid_losses2, _ = train_model(numeric_model, train_loader, valid_loader, num_classes, device, patience=8, factor=0.5,
                max_norm=0.5, alpha=None, gamma=2, weight_decay=1e-4, num_epochs=num_epochs, learning_rate=lr,
                save_model_name="",mode="numeric")
    # 损失绘制
    plot_loss_curves(train_losses1, valid_losses1,train_losses2, valid_losses2,
                     f"multi_soft_loss")

    metrics_train, preds_train, labels_train = evaluate_seq2seq_model(img_model, numeric_model,device, train_loader)
    start = time.time()
    metrics_valid, preds_valid, labels_valid = evaluate_seq2seq_model(img_model, numeric_model, device, valid_loader)
    end = time.time()
...
if __name__ == '__main__':
    ...
    for random_seed in [180, 878, 655, 672, 437, 77, 154, 766, 722, 426, 522, 458, 265, 265, 14, 714, 683, 296, 25, 186]:
    # for random_seed in [random.randint(0, 1000) for _ in range(50)]:
        metric = main(random_seed=random_seed,window_size=window_size,stride=stride)
        print('*'*100)
        metric_list.append(metric)
    metrics_df = pd.DataFrame(metric_list)
    metrics_df.to_csv(f'./20_tril_result/multi_soft_loss_{window_size}_metrics.csv',
                      index=False)