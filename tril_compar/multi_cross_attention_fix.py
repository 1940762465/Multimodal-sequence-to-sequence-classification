...
class MultiModeSeq2SeqModel4(nn.Module):
    def __init__(self, input_size, num_classes, window_size, fuse_heads=4, ViT_feature_dim=256, hidden_dim=32,dropout=0.1,vit_type="vit_tiny_patch16_224",weight_path=None):
        super(MultiModeSeq2SeqModel4, self).__init__()
        self.window_size = window_size

        self.attention = nn.MultiheadAttention(embed_dim=hidden_dim, num_heads=fuse_heads)
    ...
    def forward(self, x, image_features):
        ...
        # 编码 - 保持时间维度
        encoded = self.encoder(x).transpose(1, 2)  # (batch_size, window_size, hidden_dim)

        # 解码 - 直接预测每个时间步
        image_expand_features = self.image_expand_windows(image_features).transpose(1,
                                                                                    2)  # (batch_size, window_size, hidden_dim)

        # 模态融合
        attn_output, attn_output_weights = self.attention(encoded.transpose(0, 1),
                                                          image_expand_features.transpose(0, 1),
                                                          image_expand_features.transpose(0,
                                                                                          1))  # (seq_len, batch_size, hidden_dim)
...
def main(
    ...
    model = MultiModeSeq2SeqModel4(input_size, num_classes, window_size,fuse_heads=4,                                   ViT_feature_dim=hidden_dim, hidden_dim=hidden_dim,
